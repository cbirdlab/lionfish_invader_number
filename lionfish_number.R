...
library(ggplot2)
#### Source Functions ####
source("/work/hobi/mfrench1/DIS/model_functions.R")
source("/work/hobi/mfrench1/DIS/demographic_parameters.R")


#### User Inputs for lionfish_number.R ####
np <- 299 #Number of processors to use
BS <- 10000 #Number of simulations to run with each source
RUN.MONTH <- 12*15     # Number of 12 months * number of years to run model
initial.females <- seq(10,10+18,by=1) # vector of number of starting female lionfish
...
#### Build MPI Cluster ####
#cluster <- makeMPIcluster(np,outfile = "/home/jselwyn/Lion/debug.txt")
cluster <- makeCluster(np,type='SOCK')
...
for(v in var.rpr){
  ...
  for(S in 1:length(sources)){
    ...
    save.image(paste('/home/jselwyn/Lion/model_images/',names(sources)[S],'-rpr-',v,'_source.RData',sep=''))
    ...
    # pdf(paste('/home/jselwyn/Lion/Graphs/Simulation/',names(sources)[S],'-plots.pdf',sep=''),width=100,height=100,onefile = T)
    # 
    ...
    pdf(paste('/work/hobi/mfrench1/DIS/',names(sources)[S],'-rpr-',v,'-statistical_plots.pdf',sep=''),onefile = T)
    ...
    for(D in 1:length(destinations)){
      ...
      write.csv(statistical.plots[[6]],paste('/work/hobi/mfrench1/DIS/',names(sources)[S],'-to-',names(destinations)[D],'-rpr-',v,'-plot.data.csv',sep=''))
    }
